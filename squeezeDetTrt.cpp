...
const char* INTERPRET_INPUT_BLOB_NAME0 = "class_slice";
const char* INTERPRET_INPUT_BLOB_NAME1 = "confidence_slice";
const char* INTERPRET_INPUT_BLOB_NAME2 = "bbox_slice";
...
createInterpretEngine(unsigned int maxBatchSize, IBuilder *builder, DataType dt)
{
	...
	auto data = network->addInput("", dt, DimsCHW{ 3, INPUT_H, INPUT_W});
	assert(data != nullptr);
...
void doInference(IExecutionContext& context, float* inputData, float* inputImInfo, float* outputBboxPred, float* outputClsProb, float *outputRois, int batchSize)
{
    ...
    Tensor *convOutput = createTensor(buffers[outputIndex0], 4, dims);
    Tensor *class_slice = sliceTensorCuda(convOutput, 3, 0, 27);
    Tensor *confidence_slice = sliceTensorCuda(convOutput, 3, 27, 9);
    Tensor *bbox_slice = sliceTensorCuda(convOutput, 3, 36, 36);
